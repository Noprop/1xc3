...
\begin{document}
	...
	This is the general format of your LaTeX files. Press File \textgreater New, and paste the following code, compile and run the code by pressing F5 or at the top \textbf{menu} press Build and View.
    ...
    revise for your quiz, starting in 15 mins
    ...
    {\fontsize{15}{18}\selectfont This is some text with a font size of 15 pt .}
	...
	\textbf{Enumerate}: The \codebox{enumerate} environment is used to \textbf{create a numbered list}. Each item in the list is automatically assigned a number or letter. The syntax for using enumerate is as follows:
...
\begin{enumerate}
    \item The default numbering style is decimal (1, 2, 3), but you can customize it by modifying the \codebox{enumi} counter or using packages like \codebox{enumitem} for more advanced customization.
    \item hello
    \item hi
    \item c
\end{enumerate}
    
    \textbf{Itemize}: The \codebox{itemize} environment is used to \textbf{create a bulleted list}. Each item in the list is represented by a bullet point. The syntax for using itemize is as follows:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Jingze has to prepare %%%%%%%%%%%%%%%%%%%%%%%%	
	...
	This code generates a scatter plot using LaTeX and the TikZ package (\codebox{\textbackslash usepackage\{tikz\}}) (\hyperref[fig:scatter_pedram]{\textbf{Figure 1}}).
	...
	With the same format, you can make a line plot by removing the option \codebox{[only marks]} (\hyperref[fig:line_Pedram]{\textbf{Figure \ref*{fig:line_Pedram}}}):
	...
	You want to have multiple lines? Take a look at \hyperref[fig:multiple line]{\textbf{Figure \ref*{fig:multiple_line}}}:
	...
	The bar plot is usually used to show the value of different categories. Let's say if I have code using four different algorithms (A, B, C, D) computing $a \times x$ where a and x are matrices. The time taken by each algorithm can be represented by \hyperref[fig:bar_plot]{\textbf{Figure \ref*{fig:bar_plot}}}.
	...
	The stacked bar plot is used when each category also has sub-sections. Let's say if each algorithm has three parts and you want to also mention the time taken by each part in the same plot (\hyperref[fig:stacked_ped]{\textbf{Figure \ref*{fig:stacked_ped}}}).
	...
	These are just a few examples of the many areas where histograms are employed. Histograms provide a valuable tool for visualizing data distributions and are widely used across various domains for data exploration, analysis, and decision-making. \hyperref[fig:his_ped]{\textbf{Figure \ref*{fig:his_ped}}} shows how to make one.
	...
	Pie chart are used any time you want to show the percentages of items. To create a pie chart in LaTeX, you can use the \codebox{pgf-pie} package. Make sure you have it installed and then include the package in your LaTeX document (\hyperref[fig:pie_pedram]{\textbf{Figure \ref*{fig:pie_pedram}}}):
...
%%%%%%%%%%%%%%%%%%%%%% Laeek has to prepare %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	Remember to include the necessary packages in the preamble of your LaTeX document, such as \texttt{\\usepackage\{listings\}} for code formatting and syntax highlighting.
...
You can use the \codebox{tableofcontents} environment to display a table of contents before each section. Here is an example of a table of contents:

\begin{lstlisting}[style=latexstyle]
\documentclass{article}
\usepackage{blindtext}
\usepackage{titlesec}
\title{"CS 1XC3 Latex Document 1"}
\begin{document}
...
\includegraphics[scale=0.4]{TOC1.png}
	
\begin{itemize}
			\item \codebox{\textbackslash  tableofcontents}: This command is used to define a new table of contents, and it automatically detects defined sections.
			
			\item \codebox{\textbackslash  section}: This is a defined ordered section.
			
			\item \codebox{\textbackslash  section*}: This is a defined unordered section.
			
		\end{itemize}

We can add subsections (as an inner component of sections) as well:
...
\includegraphics[scale=0.6]{TOC2.png}

\begin{itemize}
			
	\item \codebox{\textbackslash  subsection}: This is a defined  subsection.
			
		\end{itemize}

Title of tables of contents are able to change as well:
\begin{lstlisting}[style=latexstyle]
\documentclass{article}
\usepackage{blindtext}
\usepackage{titlesec}
\title{"CS 1XC3 Latex Document 1"}
\begin{document}
\renewcommand*\contentsname{Jingze's Summary}
...
\includegraphics[scale=0.6]{TOC3.png}

\begin{itemize}
			
	\item \codebox{\textbackslash  renewcommand\* \textbackslash  contentsname}: This environment sets up tables of contents' titles. The default value is \codebox{Contents}
...
\subsection{Linking to a Website}
You can link to a website using the \texttt{href} command from the \texttt{hyperref} package. For example, to link to the GitHub website, use the following code:

\href{https://github.com}{Github}

\subsection{Linking to a Part of the Document}
To link to a specific part of the document, you need to set a \texttt{label} for the target location and then use the \texttt{ref} command to refer to it. Here's an example:

To link to a specific part of the document, you need to set a \texttt{label} for the target location and then use the \texttt{ref} command to refer to it. Here's an example:

You can refer back to the \textbf{Tables of Contents} section by clicking \hyperref[sec:toc]{\textbf{here}}.

\subsection{Mentioning an Article as a Reference}
To create a bibliography and cite an article as a reference, you should use the \texttt{cite} command in combination with a bibliography style and the \texttt{bibliography} command. Here's a simple example:
